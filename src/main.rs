...
use crate::{boat::Boat, boat_info::{BoatInfo, NavigationData, StaticData, VoyageData}, common::utils::log};
...
async fn main() {
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            let static_data: StaticData = boat_info.get_static_data();
            let voyage_data: VoyageData = boat_info.get_voyage_data();
            let nav_data: NavigationData = boat_info.get_navigation_data();
            ...
            let ui_handle_clone: slint::Weak<AppWindow> = ui_handle.clone();

            let _ = slint::invoke_from_event_loop(move || {
                if let Some(ui) = ui_handle_clone.upgrade() {
                    let boat_data = ui.global::<BoatData>();
                    let logs: ModelRc<SharedString> = get_last_logs_entries(300);
